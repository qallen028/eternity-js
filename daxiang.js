...
async function category(inReq) {
	...
	return {
		page: page,
		pagecount: (xiyueta('.hl-page-total').text().match(/\d+/g) && parseInt(xiyueta('.hl-page-total').text()
			.match(/\d+/g)[1])) || 1,
		list: books,
	};
}
...
async function detail(inReq) {
	...
	// 获取播放列表
	let tabs = [];
	xiyueta('.hl-plays-from a').each(function() {
	...
	for (let i = 0; i < tabs.length; i++) {
		let urls = [];
		xiyueta('.hl-plays-list').eq(i).find('li a').each(function() {
